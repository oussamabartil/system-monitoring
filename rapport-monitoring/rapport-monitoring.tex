...
\textbf{Note :} Les graphiques détaillés de performance seront générés automatiquement une fois que les dépendances Python (matplotlib, seaborn, pandas, numpy) seront installées. Pour l'instant, des placeholders sont utilisés.
...
\centering
\vspace{2cm}
\textbf{\Large Graphique d'Utilisation CPU}\\[1cm]
Ce graphique montrera l'utilisation CPU sur 24 heures avec :\\
• Courbe temporelle de l'utilisation\\
• Seuils d'alerte (30\% et 85\%)\\
• Zones d'alerte colorées\\
• Pics de charge identifiés
\vspace{2cm}
\end{minipage}}
\caption{Utilisation CPU sur 24 heures (placeholder)}
\label{fig:cpu-usage}
\end{figure}
...
\centering
\vspace{2cm}
\textbf{\Large Graphique d'Utilisation Mémoire}\\[1cm]
Ce graphique montrera :\\
• Mémoire utilisée vs libre en MB\\
...
\begin{lstlisting}[language=yaml, caption=Exemple de règle d'alerte CPU]
- alert: HighCPUUsage
...
Pour vérifier le fonctionnement de base du système :

\begin{lstlisting}[language=powershell, caption=Test de base du système]
...
\begin{lstlisting}[language=powershell, caption=Test des alertes]
# Lancer un test de charge CPU
.\cpu_stress_test.ps1

# Envoyer une alerte de test
.\scripts\test-mailtrap.ps1
\end{lstlisting}
...
Write-Host "Accès aux interfaces :" -ForegroundColor Yellow
...
\begin{itemize}
    \item Documentation officielle Prometheus : \url{https://prometheus.io/docs/}
    \item Documentation officielle Grafana : \url{https://grafana.com/docs/}
    \item Documentation AlertManager : \url{https://prometheus.io/docs/alerting/latest/alertmanager/}
    \item Guide Docker Compose : \url{https://docs.docker.com/compose/}
    \item Mailtrap Documentation : \url{https://mailtrap.io/blog/}
\end{itemize}
